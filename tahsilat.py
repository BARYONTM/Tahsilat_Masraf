...
class TahsilatWindow(QDialog):
    ...
    def initUI(self):
        ...
        # Ortada Tahsilat makbuzu yazısı
        title_label = QLabel(self)
        title_label.setText('Tahsilat Makbuzu')
        ...
        save_button.clicked.connect(self.save_tahsilat)
    ...
    def save_tahsilat(self):
        ...
        # Tahsilatı kaydetme işlemi burada gerçekleştirilir
        with open('tahsilat_kayitlar.csv', 'a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([firma, tutar, doviz, odeme, aciklama])

        QMessageBox.information(self, 'Kaydet', 'Tahsilat kaydedildi.')

    def show_kayitlar(self):
        self.kayitlar_window = tahskayit.KayitlarWindow()
        self.kayitlar_window.exec_()

if __name__ == '__main__':
    app = QApplication([])
    window = TahsilatWindow()
    window.show()
    app.exec_()
